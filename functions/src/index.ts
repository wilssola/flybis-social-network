...
export const cspReport = functions.https.onRequest((request, response) => {
  let report = JSON.parse(request.body.toString("utf8"));
  ...
  admin
    ...
    .add(report)
    .then((doc) => {
      let result = `CSP Report with ID: ${doc.id} has been added.`;
      console.log(result);

      return response.json({ result });
    })
    .catch((error) => {
      let result = `Failed to add CSP Report.`;
...
export const onCreateActivityFeedItem = functions.firestore
  ...
  .onCreate(async (snapshot, context) => {
    ...
    // Get FCM Tokens.
    const fcmTokenRef = admin
      .firestore()
      .collection("users")
      .doc(userId)
      .collection("tokens")
      .doc("fcm");
    const doc = await fcmTokenRef.get();

    // Check if have FCM Android Token.
    const androidNotificationToken = doc.data().androidToken;
    if (androidNotificationToken) {
      // Send notification.
      sendNotification(androidNotificationToken, snapshot.data());

      console.log("With Android Notification Token", androidNotificationToken);
    } else {
      console.log("Without Android Notification Token");
    }

    // Check if have FCM Web Token.
    const webNotificationToken = doc.data().webToken;
    if (webNotificationToken) {
      // Send notification.
      sendNotification(webNotificationToken, snapshot.data());

      console.log("With Web Notification Token", webNotificationToken);
    } else {
      console.log("Without Web Notification Token");
    }

    function sendNotification(notificationTokenChild, activityFeedItem) {
      let body;
...
export const onCreateUser = functions.firestore
  ...
  .onCreate(async (snapshot, context) => {
    ...
    admin.firestore().collection("usernames").doc(doc.username).set({
      uid: uid,
    });
  });
