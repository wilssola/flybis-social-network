...
class CameraPage extends StatefulWidget {
  ...
  CameraPage({@required this.scaffoldKey, this.currentUser, this.pageColor});
...
class CameraState extends State<CameraPage>
  ...
  File ioFile;
  Uint8List webFile; //html.File webFile;
  ...
  Future filterImage(context) async {
    var fileName = ioFile.path.split('/').last;
    var image = Im.decodeImage(ioFile.readAsBytesSync());
    ...
    if (imagefile != null && imagefile.containsKey('image_filtered')) {
      if (mounted) {
        setState(() {
          ioFile = imagefile['image_filtered'];
        });
      }
    }
  }
  ...
  handleSubmit() async {
    ...
    if (contentType == 'video') {
      contentUrl = await _takeVideo(ioFile);
    } else {
      //await compressImage();
      contentUrl = await uploadFile(ioFile);
    }
  ...
  compressImage() async {
    ...
    Im.Image imageFile = Im.decodeImage(ioFile.readAsBytesSync());
