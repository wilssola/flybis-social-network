...
import 'package:flybis/models/Profile.dart';
...
class CreateProfileState extends State<CreateProfile> {
  ...
  photoFromGallery() async {
    var result = await widget.profile
        .photoGallery(widget.userId, 'banner-${widget.userId}');
  ...
  photoFromCamera() async {
    var result = await widget.profile
        .photoCamera(widget.userId, 'banner-${widget.userId}');
                ...
                children: <Widget>[
                  ...
                  inputForm(
                    ...
                    validatorCustom: (String value) {
                      usernamesRef
                          .document(value.trim().toLowerCase())
                          .get()
                          .then(
                        (doc) {
                          if (doc.exists) {
